...
class CSVSegReader(object):
    ...
    def get_batch(self, batch_size=1):
        ...
        if self.random:
            image_batch, seg_batch, filename_batch = tf.train.shuffle_batch([image, seg, file_name], batch_size=self.batch_size,
                                                            num_threads=self.num_threads,
                                                            capacity=self.capacity,
                                                            min_after_dequeue=self.min_after_dequeue)
        else:

            image_batch, seg_batch, filename_batch = tf.train.batch_join([(image, seg, file_name)], batch_size=self.batch_size,
                                                            capacity=self.capacity, allow_smaller_final_batch=True)
