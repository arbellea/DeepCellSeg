...
from RNNSeg.Params import ParamsEvalBiLSTM
...
def run_net():
    ...
    # Build Network Graph
    net_fw = BiLSTMNetwork()
    net_bw = BiLSTMNetwork()
    ...
    with tf.device('/gpu:0'):
        with tf.name_scope('run_tower'):
            ...
            with tf.variable_scope('net'):
                _ = net_fw.build(image_seq_norm_fw, phase_train=True, net_params=params.net_params, max_len=4)
            with tf.variable_scope('net', reuse=True):
                _ = net_bw.build(image_seq_norm_bw, phase_train=True, net_params=params.net_params, max_len=4)
    ...
    with tf.Session(config=config) as sess:
        ...
        while loop:
            try:
                ...
                fetch_out = sess.run([fw_outputs, bw_outputs, net_fw.states[-1], net_bw.states_back[-1], filename_seq_fw, filename_seq_bw, sigout, image_seq_fw],
                                     options=options, feed_dict=feed_dict)
                ...
                for state_ph, last_state in zip(net_fw.states[0], states_fw):
                    feed_dict[state_ph[0]] = last_state[0]*4/np.float32(t)
                    feed_dict[state_ph[1]] = last_state[1]
                    print(last_state[0].max(), last_state[0].min())
                    print(last_state[1].max(), last_state[1].min())
