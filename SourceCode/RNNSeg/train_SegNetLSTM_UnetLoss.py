...
from RNNSeg.Params import ParamsLSTM_Unet as ParamsLSTM
...
def train():
    ...
    with tf.name_scope('Data'):
        (train_image_seq_batch, train_seg_seq_batch, train_loss_weight_seq_batch,
         train_is_last_batch) = train_data_provider.get_batch()
        val_image_seq_batch, val_seg_seq_batch, val_loss_weight_seq_batch, val_is_last_batch = val_data_provider.get_batch()
    # Build Network Graph
    net = LSTMNetwork()
    val_net = LSTMNetwork()

    def calc_losses(out_sequence, gt_sequence, weight_sequence):
        t_loss = []
        t_jaccard = []
        eps = tf.constant(np.finfo(np.float32).eps)
        with tf.name_scope('loss_calc'):
            for time_step, (out_seg, gt_seg, w) in enumerate(zip(out_sequence, gt_sequence, weight_sequence)):
                with tf.name_scope('timestep-{}'.format(time_step)):
                    ...
                    gt_weights = tf.to_float(tf.squeeze(w, axis=1))
                    gt_valid = tf.multiply(tf.to_float(tf.greater(gt_labels, -1)), gt_weights+1)
                    ...
                    t_pixel_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.to_int32(gt_fg),
                                                                                  logits=out_seg)
                    ...
                    union = tf.reduce_sum(
                        tf.to_float(tf.logical_or(tf.logical_and(tf.greater(gt_labels, -1), out_fg), gt_fg)),
                        axis=(1, 2), name='union')
    ...
    with tf.device('/gpu:0'):
        with tf.name_scope('train_tower'):
            ...
            if params.one_seg:
                train_loss, train_jaccard = calc_losses([net_segs[-1]], train_seg_seq_batch,
                                                        train_loss_weight_seq_batch)

            else:
                train_loss, train_jaccard = calc_losses(net_segs, train_seg_seq_batch, train_loss_weight_seq_batch)
        ...
        with tf.name_scope('val_tower'):
            ...
            if params.one_seg:
                val_loss, val_jaccard = calc_losses([valnet_segs[-1]], val_seg_seq_batch, val_loss_weight_seq_batch)
            else:
                val_loss, val_jaccard = calc_losses(valnet_segs, val_seg_seq_batch, val_loss_weight_seq_batch)
    ...
    tf.summary.image('Segmentation', tb_image[:,:,:,1:2], max_outputs=1, collections=['train_summaries'])
    ...
    with tf.name_scope('tb_val'):
        ...
        tb_image = tf.nn.softmax(tb_image)
        tf.summary.image('Segmentation', tb_image[:,:,:,1:2], max_outputs=1, collections=['val_summaries'])
    ...
    with tf.Session(config=config) as sess:
        ...
        while t < params.num_iterations:
            try:
                ...
                if not t % 10:
                    print('Iteration {0}: Loss: {1:.3f}, Jaccard: {2:.3f}, Time: {3:.1f} seconds. '.format(t,
                                                                                                           train_loss_eval,
                                                                                                           train_jaccard_eval,
                                                                                                           elapsed_time / 10.0, ))
                    elapsed_time = 0.
