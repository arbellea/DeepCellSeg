...
from RNNSeg.Isbi_Params import ParamsEvalIsbiLSTM_Unet
        ...
        with tf.name_scope('run_tower'):
            ...
            with tf.variable_scope('net'):
                outputs = net.build(image_seq_norm, phase_train=True, net_params=params.net_params,
                                    data_format=params.data_format, unet_loss=True)
        ...
        while loop:
            try:
                ...
                if not params.dry_run:
                    ...
                    sigoutnp_rgb = np.concatenate((sigoutnp, np.zeros(sigoutnp.shape[:2] + (1,))), 2)
                    sigoutnp_vis = np.flip(np.round(sigoutnp_rgb * (2 ** 16 - 1)).astype(np.uint16), 2)
...
if __name__ == '__main__':
    ...
    params = ParamsEvalIsbiLSTM_Unet(args_dict)
    run_net()
